...
import { View, Text, TouchableOpacity, TextInput, Platform, StyleSheet, StatusBar, Alert, Button, Dimensions, FlatList } from 'react-native';
...
const NutritionForm = (props) => {
  ...
  const deleteNutrition = () => {
    axios.delete(`${baseURL}Nutritions/${id}`)
   .then((res) => {
  ...
  const addNutrition = () => {
    ...
    if(id !== "") {
        axios
        .put(`${baseURL}Nutritions/${id}`, nutrition)
        .then((res) => {
        ...
        .catch((error) => {
            ...
            axios
            .post(`${baseURL}Nutritions/add`, nutrition)
            .then((res) => {
                if (res.status == 200) {
                console.log("Success")
                    props.navigation.navigate("Nutrition");
  ...
  return (
    <View style={styles.container}>
        ...
        >
            <Text style={[styles.text_footer, {color: colors.text}]}>Meal</Text>
